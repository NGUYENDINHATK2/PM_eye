...
export function Sidebar({ userEmail }: { userEmail?: string }) {
  ...
  return (
    <aside className="w-[260px] shrink-0 h-screen sticky top-0 glass border-r flex flex-col">
      {/* Brand */}
      ...
      {/* Footer stat card */}
      <div className="p-3">
        <div className="relative rounded-xl border bg-gradient-to-br from-indigo-500/[0.06] via-transparent to-violet-500/[0.08] p-3 overflow-hidden">
          <div className="absolute -top-6 -right-6 w-20 h-20 rounded-full bg-indigo-500/20 blur-2xl pointer-events-none" />
          <div className="relative">
            <div className="eyebrow text-[9px]">Mẹo nhanh</div>
            <div className="text-xs mt-1 leading-relaxed">
              Kéo slider phân bổ &gt; 100% sẽ <strong className="gradient-text">cảnh báo overload</strong> ngay.
            </div>
          </div>
        </div>
      </div>
...
function NavLink({ item, active }: { item: Item; active: boolean }) {
  ...
  return (
    <Link
      href={item.href}
      className={cn(
        "group relative flex items-center gap-3 h-9 mx-1 px-3 rounded-lg text-sm transition-all",
        active
